...
/// Pumps [MaterialLogViewer] constrained to [width] — simulating it being
/// docked in a side panel (or a narrow window) of that width, rather than
/// resizing the whole test surface. Wrapped in a bare [Material] (not a
/// [Scaffold]) — see the equivalent helper in material_log_viewer_test.dart
/// for why.
...
void main() {
  ...
  group('below the master-detail breakpoint', () {
    testWidgets('shows only the list, opening a bottom sheet on tap',
        (tester) async {
  ...
  group('at or above the master-detail breakpoint', () {
    ...
    testWidgets('tapping a row updates the panel instead of opening a sheet',
        (tester) async {
      buffer.capture(
      ...
      expect(find.byType(LogEntryDetailSheet), findsNothing);
      expect(
        find.descendant(
          of: find.byType(LogEntryDetailPanel),
