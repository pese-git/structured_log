...
- ~~Точная версия `fluent_ui`~~ — решено на момент реализации: `4.15.1`, точно (не диапазоном). Последняя опубликованная (`4.16.1`) не компилировалась с используемым тогда Flutter SDK (апстрим-рассинхрон API самого `fluent_ui` с недавним Flutter framework — `RawTooltip.ignorePointer`, `ReorderableListView.builder.onReorderItem`, тип `ScrollCacheExtent` не найден); `flutter analyze` этого не ловил, только `flutter test`/`flutter build`. `4.15.1` компилировался и проходил все тесты.
  **Обновление (2026-09):** причина была не в `fluent_ui`, а в устаревшем локальном FVM-пине Flutter SDK в этом репозитории (`3.41.7`) — `fluent_ui` с `4.16.0` требует Flutter `3.44.0+` (см. его CHANGELOG: «refactor: Flutter 3.44.0 support»), просто не отразил это в собственном `environment.flutter` констрейнте. После обновления `.fvm/fvm_config.json` до `3.44.9` пакет `4.16.1` компилируется и проходит все тесты — точный пин снят, теперь `fluent_ui: ^4.16.1`. См. [AGENTS.md](../../AGENTS.md).
