...
- Структура репозитория: `git mv` существующего пакета в `packages/structured_log/`; корневой `melos.yaml` становится манифестом workspace (`packages: [packages/**]`); пути в `AGENTS.md`/`CLAUDE.md` обновляются.
- Новые пакеты: `packages/structured_log_flutter/` и `packages/structured_log_material/` (у каждого свои pubspec, `lib/`, `test/`, `example/`).
