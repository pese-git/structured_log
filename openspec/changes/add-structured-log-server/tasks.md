...
- [ ] 5.1 `POST /v1/users`, `GET /v1/users` (admin only); `POST /v1/users/:id/block` (`is_active = false`, инкремент `token_version` — 4.2, отзыв всех `refresh_tokens` пользователя) и `POST /v1/users/:id/unblock` (`is_active = true`) — admin only (4.5)
