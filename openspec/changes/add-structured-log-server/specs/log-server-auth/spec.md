...
Сервер SHALL предоставлять `DELETE /v1/users/me`, доступный любому аутентифицированному пользователю (`admin`/`owner`/`user`) исключительно над собственной учётной записью, принимающий JSON-тело `{"password": "..."}` с текущим паролем как подтверждением. При совпадении пароля сервер SHALL атомарно: установить `deleted_at` (текущее время) и `is_active = false`, отозвать все refresh-токены этого пользователя, инкрементировать `token_version`. Удаление SHALL быть необратимым через API — сервер не SHALL предоставлять способ снять `deleted_at`.

#### Scenario: Успешное удаление собственного аккаунта завершает текущую сессию
- **WHEN** аутентифицированный пользователь отправляет `DELETE /v1/users/me` с верным текущим паролем
...
#### Scenario: Удалить можно только собственный аккаунт через этот эндпоинт
...
- **THEN** удалён может быть только сам вызывающий, независимо от его роли — `admin`, желающий удалить чужой аккаунт, не имеет для этого отдельного эндпоинта в рамках этой доработки
...
- **WHEN** `admin` отправляет `POST /v1/users/:id/unblock` для пользователя, самостоятельно удалившего свой аккаунт
