...
- [x] 2.2 `deploy/proxy/nginx.conf.template`: `X-Content-Type-Options` and `Referrer-Policy` on the `/v1/` location only — the API's responses never pass through the `web` image. Nothing on `/`, so no header is sent twice.
