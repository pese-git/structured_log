...
`--tag` можно указать несколько раз. «Коммит, и ещё `latest`» — это одно
решение и два имени для одного образа, а два запуска скрипта такого не
дадут: две сборки одного коммита — не одни и те же байты (один только
web-бандл невоспроизводим), так что `latest` указывал бы на образ,
который никто не проверял. Повторённый `--tag` собирает один раз и вешает
на результат все имена.
...
Первый `--tag` — тот, который цитирует итоговая подсказка про kustomize:
запуск с коммитом и `latest` означает коммит, а `latest` при нём —
подвижный псевдоним.
...
`--base-href` — путь, под которым отдаётся admin-клиент (по умолчанию `/`).
Он зашивается в бандл как `<base href>`, и относительно него загружаются
все скрипты, манифест и иконки — поэтому Ingress, монтирующий клиент под
префиксом (и отрезающий его до того, как запрос дойдёт до пода), требует
того же префикса при сборке:
...
Ошибка здесь не мешает загрузиться самой странице, но та просит
`/flutter_bootstrap.js` из корня; то, что обслуживает `/`, отвечает 404
HTML-страницей, и браузер сообщает об ошибке MIME-типа, а не об
отсутствующем файле. Readiness-проба этого не видит — она запрашивает `/`,
а он по-прежнему работает.
