...
работать только в одной реплике, почему имя Service `server` критично,
три реальные ловушки, найденные при сборке этого) — см.
...
просто написанный по чтению конфига.
...
не передан явный `--rotate`, ровно затем, чтобы это нельзя было по
привычке разлогинить все сессии в кластере. Если у вас уже есть
...
(`logs.example.com`) и направляет всё на Service `web` — его
собственный nginx уже разделяет `/v1/` на API внутри, так что сам
`Ingress` остаётся простым. Отредактируйте поле `host:` под свой домен,
либо переопределите его per-overlay небольшим Kustomize-патчем, если
