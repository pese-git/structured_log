...
void main() {
  ...
  group('insertBatch', () {
    test(
      'ties every entry to the given project and returns the stored rows',
      () async {
  ...
  group('insertBatch, as the ingest path uses it', () {
    ...
    test(
      'returns rows in order, with consecutive ids, exactly as stored',
      () async {
        ...
        final rows = await recordedStore.insertBatch(project, [
          for (var i = 0; i < 25; i++)
        ...
        final stored =
            await (recorded.select(recorded.logEntries)..where(
                  (t) => t.id.isBetweenValues(rows.first.id, rows.last.id),
                ))
                .get();
        expect(rows, stored);
    ...
    test('inside the caller\'s transaction it opens none of its own', () async {
      await recorded.transaction(() async {
        await recordedStore.insertBatch(project, [
          entry(event: 'a'),
          entry(event: 'b'),
        ]);
      });
    ...
    test(
      'outside a transaction it runs in one of its own, and only one',
      () async {
    ...
    test('the number of statements does not grow with the batch', () async {
      Future<int> statementsFor(int n) async {
        ...
        await recorded.transaction(() async {
          await recordedStore.insertBatch(project, [
            for (var i = 0; i < n; i++) entry(event: 'x$i'),
          ]);
        });
    ...
    test(
      'is atomic on its own: a batch that cannot be stored stores nothing',
      () async {
        await expectLater(
          recordedStore.insertBatch(99999, [
            entry(event: 'a'),
            entry(event: 'b'),
          ]),
          throwsA(anything),
    ...
    test('two concurrent batches each get back their own rows', () async {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        expect(
          results[i].every((r) => r.projectId == (i.isEven ? project : other)),
          isTrue,
        );
      }
  ...
  group('query', () {
    ...
    test(
      'a cursor page contains only strictly older entries, no overlap',
      () async {
        ...
        expect(
          secondPage.entries.every((e) => e.id < firstPage.entries.last.id),
          isTrue,
        );
      },
    );
