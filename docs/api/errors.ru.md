...
| 401 | `unauthorized` | общий | Любой JWT-защищённый эндпоинт | Заголовок `Authorization` отсутствует/некорректен; подпись JWT невалидна либо истёк её срок действия; claim `tv` больше не совпадает с `User.token_version` ([auth.md](../architecture/auth.ru.md#token_version-как-снапшот-в-jwt-остаётся-отзываемым)); либо вместо access-токена предъявлен секретный ключ проекта |
...
| 403 | `cannot_delete_primary_admin` | общий | `DELETE /v1/users/me`, `DELETE /v1/users/:id`, `DELETE /v1/teams/:teamId/members/:userId`, `DELETE /v1/role-assignments/:id` | У цели `is_primary_admin = true` — см. [rbac-and-lifecycle.md](../architecture/rbac-and-lifecycle.ru.md) |
...
| 429 | `too_many_subscriptions` | general | `GET /v1/logs/stream` | Потолок живых подписок исчерпан — собственный у учётной записи или общий на процесс (`--max-live-subscriptions-per-user`, `--max-live-subscriptions`); `details` называет заданные пределы. **Без `Retry-After`**, в отличие от всех прочих 429: здесь ждать приходится, пока кто-то закроет соединение, и честного момента сервер назвать не может |
...
данные (id проектов/групп видны большему числу людей, чем тех, у кого
есть права запроса на них — например, всем в той же организации).
...
## `429` — единственный ответ token-эндпоинта не в RFC-форме

`POST`/`DELETE /v1/auth/token` отвечает в конверте RFC 6749 §5.2 на всё,
что порождает сам, — но запрос, остановленный ограничителем частоты, до
обработки гранта не доходит вовсе. Его отклоняет middleware, ровно в той
позиции, которую занял бы обратный прокси, поставь его оператор перед
сервером, — поэтому ответ приходит в общем конверте, как у любого
другого эндпоинта:
...

{
  ...
  "message": "слишком много попыток, повторите через 43 секунды"
}
```

Втиснуть этот случай в RFC-конверт означало бы выбрать
`invalid_request` — единственный отдалённо подходящий код — и тем самым
сказать клиенту, что его совершенно правильный запрос неправильный.
Клиент отличает этот случай по HTTP-статусу, а не по форме тела:
конформный OAuth2-клиент и так обязан отличать `400` с телом ошибки от
любого другого статуса, а `Retry-After` — стандартный HTTP-заголовок
(RFC 9110), а не изобретение этого API.

При обработке `429` важны два свойства:

- **Действие не выполнено.** Пароль не проверялся, токен не выпускался,
  письмо не отправлялось. Повтор после `Retry-After` безопасен и остаётся
  единственным способом узнать, верны ли креды.
- **Он ничего не говорит об учётной записи.** `429` на
  `password-reset`/`verify-email/resend` возвращается одинаково,
  существует отправленный адрес или нет: ограничитель ведёт счётчик по
  отправленной строке именно для того, чтобы не стать тем оракулом
  существования, которого избегают одинаковые `202`. Блокировки учётной
  записи за этим ответом тоже нет — ничего не заморожено, и доступ
  возвращается сам по истечении окна
