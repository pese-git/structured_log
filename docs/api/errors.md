...
| 429 | `too_many_subscriptions` | general | `GET /v1/logs/stream` | The live-stream ceiling is full — the account's own or the process's (`--max-live-subscriptions-per-user`, `--max-live-subscriptions`); `details` names whichever limits are set. **No `Retry-After`**, unlike every other 429 here: this waits on somebody closing a connection, and the server has no honest moment to name |
