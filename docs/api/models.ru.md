...
Это не дословные цитаты из `specs/*.md` — требования OpenSpec описывают
*поведение* (например, «JSON-объект, идентифицирующий ошибку»), не
всегда фиксируя имена полей. Этот документ закрепляет одну конкретную,
согласованную форму для всех них, следуя
...
| `secret` | string | **Только в ответе на создание** — значение в открытом виде, показывается один раз; всегда с префиксом `slk_` |
...
// action: "auth.login_failed" — известная учётная запись
{"reason": "invalid_password", "client_ip": "203.0.113.7", "user_agent": "..."}

// action: "auth.login_failed" — такой учётной записи нет (actor_user_id — null,
// отправленный username сознательно НЕ сохраняется)
