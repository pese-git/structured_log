...
| Один писатель-`QueryExecutor`, WAL и пул isolate-читателей (`--db-read-pool-size`) | Isolate на запрос / шардирование | `shelf` уже обслуживает запросы в одном isolate по умолчанию — единственный писатель не требует дополнительной синхронизации, и масштабирование сверх этого остаётся явным non-goal. Чтения раньше делили с ним то же соединение, поэтому чтение стояло в очереди за тем, что в этот момент писал приём. WAL позволяет пулу дополнительных соединений, каждое на своём isolate, обслуживать `SELECT` вне транзакции, пока единственный писатель ведёт всё, что меняет данные (`AGENTS.md`). Сам приём: параллельные `POST /v1/logs` тоже больше не открывают каждый свою транзакцию — запрос, пришедший пока идёт коммит, присоединяется к следующему, из-за чего одиночный запрос перестал платить ~0,65 мс фиксированных накладных расходов транзакции целиком, разделив их с пачкой (`AGENTS.md`, `IngestCoordinator`). Ни одно из двух изменений не трогает, почему [broadcast живого потока](live-streaming.ru.md) остаётся in-process `StreamController`: каждая принятая вставка по-прежнему проходит через единственного писателя, в одном процессе, есть пул читателей или нет. Эта конфигурация «один писатель + пул читателей» целиком специфична для SQLite — выбираемая оператором альтернатива в следующей строке. (decision 4) |
| `drift_postgres` (`PgDatabase`) как выбираемый оператором альтернативный backend (`--db-backend=postgres`) | Обязательный PostgreSQL всегда, либо второй слой хранения с нуля | SQLite остаётся дефолтом — без настройки, без отдельного сервиса. PostgreSQL — opt-in для операторов, у которых он уже есть и кому нужны его конкурентные писатели (MVCC) вместо обходного пути SQLite «один писатель + WAL», либо хранение логов на уже существующей инфраструктуре backup/HA. Переиспользует одну схему `drift` для обоих диалектов (генерируется для обоих через `sql: dialects:` в `build.yaml`), а не дублирует модели — каждое реальное различие (синтаксис плейсхолдера в сыром SQL, извлечение JSON-пути, булев литерал частичного индекса, `RETURNING` против `last_insert_rowid()` при батч-вставке) — небольшая ветка по `executor.dialect`, не параллельная реализация. Никогда не переключается на лету и никогда не мигрирует данные автоматически SQLite→Postgres — выбирается один раз, при развёртывании, на пустой базе. См. [data-model.md](data-model.ru.md#postgresql-выбираемый-оператором-альтернативный-backend) и [openspec/changes/add-postgres-backend/](../../openspec/changes/add-postgres-backend/) для полной записи решения, включая два дефекта, найденных только запуском настоящего сервера против настоящего Postgres (зависание при остановке из-за незакрытого пула соединений, и необходимость `--concurrency=1` для Postgres-тегированных тестовых файлов). (add-postgres-backend decisions 1–10) |
