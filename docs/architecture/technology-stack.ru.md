...
| `drift_postgres` (`PgDatabase`) как выбираемый оператором альтернативный backend (`--db-backend=postgres`) | Обязательный PostgreSQL всегда, либо второй слой хранения с нуля | SQLite остаётся дефолтом; PostgreSQL — opt-in для операторов, которым нужен MVCC или уже есть инфраструктура backup/HA — полное сравнение backend'ов см. в [data-model.md](data-model.ru.md#postgresql-выбираемый-оператором-альтернативный-backend). (add-postgres-backend decisions 1–10) |
...
`drift_postgres` переиспользует одну схему `drift` для обоих диалектов
(генерируется для обоих через `sql: dialects:` в `build.yaml`), а не
дублирует модели — каждое реальное различие представляет собой
небольшую ветку по `executor.dialect`, не параллельную реализацию:

- **Синтаксис плейсхолдера в сыром SQL** различается напрямую (`?` у
  SQLite против собственных позиционных параметров Postgres).
- **Извлечение JSON-пути** отличается по диалекту.
- **Булев литерал частичного индекса** отличается по диалекту.
- **Батч-вставка** использует `RETURNING` на Postgres против
  `last_insert_rowid()` на SQLite.

Выбор никогда не переключается на лету и никогда не мигрирует данные
автоматически SQLite→Postgres — он делается один раз, при
развёртывании, на пустой базе. Полная запись решения — включая два
дефекта, найденных только запуском настоящего сервера против
