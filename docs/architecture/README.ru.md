...
- **Никакой магии, никакого codegen сверх одного осознанного
  исключения.** У воркспейса уже есть устоявшийся принцип «никакой
  магии» (`add-structured-log-flutter/design.md`, decision 4); выбор
  `shelf`/`shelf_router` вместо `dart_frog` следует ему (decision 1).
  `drift` (и, следовательно, `build_runner`) — *единственное*
  санкционированное исключение, ограниченное storage-слоем
  `structured_log_server`, принятое по прямому указанию пользователя
  (decision 2) — не прецедент для codegen где-либо ещё в воркспейсе.
