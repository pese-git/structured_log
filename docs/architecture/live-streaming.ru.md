...
Передача буфера содержит ту же гонку в миниатюре. Доставка буферизованного
события может ждать (событие проекта, о котором сервер ещё не спрашивал, стоит
чтения из базы), и пока она ждёт, приходят и буферизуются новые события.
