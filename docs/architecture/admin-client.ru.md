...
Workspace шире, чем этот клиент: отдельно от него он поставляет ещё
три пакета виджетов для *просмотра* логов во Flutter-приложениях
(`structured_log_flutter`/`material`/`fluent` — к этому admin-клиенту
отношения не имеют). Они разделены на headless-ядро
(`LogViewerController`/`LogBuffer`) и скины под конкретную
дизайн-систему — разделение оправдано тем, что скинов там больше
одного. Этот паттерн сюда не подходит (decision 18): весь смысл
...
У него также нет **никакого общего Dart-кода** с `structured_log_server` —
...
- **`domain`** — сущности и *интерфейсы* репозиториев, без импортов
  ...
  тестируемой без сборки виджетов или запуска реального сервера.
...
- **Почему отдельный пакет, а не папка `lib/shared/widgets/` внутри
  ...
  ничто не мешает файлу простого («глупого») виджета случайно
  импортировать `Bloc` или репозиторий — сразу или со временем, когда экран
...
- **Три уровня, не классические пять.** Atomic Design (Brad Frost) также
  ...
  бизнес-логику конкретной фичи. Ровно этого в пакете быть не должно,
  поэтому `templates`/`pages` остаются там, где данные и
...
- **Дублированные, не общие цвета уровня лога.** Атом `LogLevelBadge`
  владеет собственным маленьким соответствием «уровень → цвет», не
  импортирует его из `structured_log_material` или
  `structured_log_fluent` — decision 21 уже исключает общий код с
  viewer-пакетами (другой источник данных), и это следует тому же
  прецеденту, что уже
...
- **`example/` — галерея компонентов** — каждый atom/molecule/organism,
  ...
  Design (decision 38).
- **Разве это не «абстракция раньше второго потребителя»** (принцип,
  стоящий за decisions 18/21 в других местах этой системы)? Нет — тот принцип про
...
`structured_log`, на это приложение не распространяется — это
приложение верхнего уровня, а не библиотека, от которой транзитивно
зависит что-то ещё.
...
удалённые, фильтруются сервером и подгружаются постранично: этого
достаточно, чтобы отклонить подгонку `LogViewerController` под оба
источника как усложнение стабильного, уже опубликованного API ради
одного нового потребителя. Автомат состояний ниже реализован как
`LogFeedBloc` (`flutter_bloc`, decision 36) — конкретный механизм,
стоящий за «собственным маленьким слоем состояния» из decision 21,
каждое состояние ниже — вариант `freezed`-объединения (decision 34).
...
- **Позиция прокрутки** неявно управляет автослежением — прокрутку
  вверх для чтения истории новые поступления не сбрасывают обратно
  вниз; вместо этого появляется бейдж («N новых событий»), и нажатие на него
...
не показано с последующим отклонением — это последовательно везде
(блокировка, удаление, выдача ролей, редактирование квот), но реальный
источник истины — всегда сервер: скрытое действие, вызванное в обход
UI, всё равно отклоняется на сервере (например,
`cannot_delete_primary_admin` на обойдённом удалении основного
администратора, или `sole_group_owner`, показанный как объяснённый
конфликт, а не общая ошибка). Что именно
