...
| `is_primary_admin` | Это тот самый аккаунт, что bootstrap создал первым? | Только самый первый успешный запуск `create-admin` (decision 28) | Н/Д — ни один API-путь не устанавливает и не снимает его |
...
- `users.is_primary_admin`: уникальный частичный индекс (`WHERE
  ...
  строка никогда не сможет нести этот флаг, даже если бы в коде
  `create-admin` была ошибка (decision 28).
