...
### Запланировано: PostgreSQL как выбираемый оператором альтернативный backend

**Ещё не реализовано** — этот раздел фиксирует архитектурное решение,
принятое раньше кода, а не отгруженную возможность. Полная запись
решения (контекст, альтернативы, риски) — в
...
механизма хранения. Решение — сделать backend хранения **явной
настройкой, которую выбирает оператор при развёртывании**
(`--db-backend=sqlite|postgres`, SQLite по умолчанию, поведение не
меняется) — никогда не переключение в рантайме и никогда не
автоматическая миграция данных между ними; оператор, выбравший Postgres,
...
- **Отдельная, специфичная для Postgres реализация горячего пути
  батч-вставки.** SQLite-путь с `last_insert_rowid()` и чтением диапазона
  id (конкретный механизм за цифрами пропускной способности из
  `technology-stack.md`, пул чтения/групповой коммит) опирается на то,
  что одно соединение держит транзакцию, в которую больше никто не может
  вставить параллельно — надёжно для SQLite, но не безопасное
  предположение для Postgres в общем случае. Postgres-путь вместо этого
  делает один multi-row `INSERT ... RETURNING *`, который напрямую
  связывает возвращённые строки со вставленными, без опоры на соседство
  id.

Всё остальное, что сегодня в коде выглядит «специфичным для SQLite» —
DB-side выражение по умолчанию для `createdAt`, пара булевых литералов в
сыром SQL, одно DDL-выражение частичного индекса — деталь
переносимости с уже принятым решением в `design.md`, а не архитектурная
развилка; полный список — там.
