...
Three product guides for the `structured_log` system — the self-hosted
log server, its admin client, and the libraries that ship logs into it.
Unlike the rest of [docs/](../README.md), which explains *why* the
system is built the way it is for contributors and reviewers, these
guides explain *how to use it*, for the three people who actually do:
...
| [Developer Guide](developer-guide.md) | Whoever writes code against this system — sending logs from an application, calling the HTTP API directly, or embedding a live in-app viewer | "How do I get my application's logs into this system, and how do I read them out programmatically?" |
