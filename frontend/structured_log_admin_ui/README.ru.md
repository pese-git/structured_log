...
Значения взяты из канваса «Structured Log Admin UI», артборды которого
используют общий блок токенов. Там, где значение выведено, а не перенесено, об этом сказано
в коде:
...
- **Адаптивные раскладки** — большинство артбордов нарисованы в одной
  десктопной ширине, но узкие состояния реализованы там, где их рисует канвас
  (`GroupsNarrow.dc.html`, `LogBrowserNarrow.dc.html`), и спроектированы там,
  где нет: `AdminAppShell` сворачивает свой nav rail ниже
  `AdminBreakpoints.navRail`, а `AdminTable` ниже собственного порога ширины
  скроллится по горизонтали, а не переполняется. См. задачу 23.9 change'а
