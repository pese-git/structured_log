...
- [backend/structured_log_server/](backend/structured_log_server/) — self-hosted сервер приёма/
  ...
  ограничение частоты, очистка по retention, собственное логирование и аудит (Этап 2 — разделы
  19/29/31); не реализованы управление пользователями, команды, выдача ролей, восстановление пароля
  и подтверждение email — разделы 2–9/21/26/28/33/34 `tasks.md` в
  [openspec/changes/add-structured-log-server/](openspec/changes/add-structured-log-server/).
...
логов, живая лента; плюс разделы 20/32 Этапа 2 — экран аудита. Остался 30.3, и он ждёт раздела 18).
