...
- В `melos.yaml` заведён отдельный скрипт `test:e2e` — в общий `test` набор не входит: каждый файл
  платит за `dart run` сервера и требует сгенерированного кода сервера и клиента. **В CI набор при
  этом гоняется всегда** — джобой `e2e`; «не входит в `test`» относится к локальному прогону, а не
  к тому, что его никто не проверяет.
...
- `e2e` — `packages/e2e` целиком: система в одном дереве процессов — настоящий
  `bin/server.dart`, `structured_log` с `HttpLogOutput`, шлющие в него, и слой
  данных admin-клиента, читающий обратно. Отдельная джоба потому, что это
  единственное место, которому одновременно нужны сгенерированный код **двух**
  пакетов (сервера — его запускает харнесс, клиента — его импортирует набор) и
  Flutter SDK. `subosito/flutter-action`, `ubuntu-latest`, `dart format` +
  `flutter analyze` + `flutter test`. Локально же e2e в общий `melos run test` не
  входит — это разные вещи: в CI набор гоняется всегда, у разработчика по
  требованию (`melos run test:e2e`).
