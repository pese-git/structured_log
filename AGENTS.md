...
23.9) и `structured_log_admin_client` (разделы 11/12/14/22 — слой данных, вход, просмотр и поиск
логов, живая лента; остались экраны управления ресурсами и смены пароля — разделы 13/27/30).
`packages/` пока пуст.
...
- Приложение (`publish_to: none`), web-платформа. Готовы слой данных (раздел 11), вход (12) и
  экран логов с живой лентой (14/22); экранов управления ресурсами ещё нет (13/27).
...
- **Две ловушки `testWidgets` с `Bloc`** (с `Cubit` их нет, обе всплыли при переходе в разделе 22):
  блок, созданный в `setUp`, не обрабатывает события внутри виджет-теста — строить его надо в теле
  теста; и `Bloc.close()` внутри `testWidgets` не завершается никогда — в `tearDown` его нельзя
  ожидать. Вторая воспроизводится на двухстрочном блоке, то есть это харнесс, а не наш код.
